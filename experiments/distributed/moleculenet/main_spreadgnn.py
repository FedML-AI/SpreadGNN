...
def load_data(args, dataset_name):
    if (args.dataset != 'sider') and (args.dataset != 'clintox') and (args.dataset != 'bbbp') and \
            (args.dataset != 'bace') and (args.dataset != 'pcba') and (args.dataset != 'muv') and (args.dataset != 'tox21'):
    ...
    args.loss = torch.nn.BCEWithLogitsLoss(reduction='none')
...
if __name__ == "__main__":
    ...
    if process_id == 0:
        wandb.init(
            ...
            project="SpreadGNN",
            name="SpreadGNN" + str(args.model) + "r" + str(args.dataset) ,
            config=args
        )
