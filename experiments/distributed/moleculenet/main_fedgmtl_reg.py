...
def add_args(parser):
    ...
    parser.add_argument('--data_dir', type=str, default='./../../../data/freesolv',
                        help='data directory')
...
def create_model(args, model_name, feat_dim, num_cats, output_dim):
    ...
    if args.is_mtl:
        #Add omega correlation matrix as a model parameter
        Sigma_k = torch.rand((num_cats, num_cats), requires_grad = False,  dtype=torch.float32)
        Sigma_k = torch.mm(Sigma_k, Sigma_k.t())
        Sigma_k.add_(torch.eye(num_cats,  dtype=torch.float32))
        model.omega_corr = torch.nn.Parameter(Sigma_k, requires_grad = False)
...
if __name__ == "__main__":
    ...
    if process_id == 0:
        wandb.init(
            ...
            project="fedmolecule",
            name="FedMolecule(d)" + str(args.lr) + "_" + args.model + "_" + args.dataset,
            config=args
        )
    ...
    trainer.task_reg = args.task_reg
