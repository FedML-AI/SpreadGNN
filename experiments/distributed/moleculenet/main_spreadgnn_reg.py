...
from SpreadGNN.SpreadGNN_API import FedML_init, FedML_SpreadGNN_distributed
...
def add_args(parser):
    ...
    parser.add_argument('--task_reg', type=float, default=0.01, help='Task regularizer coefficient')

    parser.add_argument('--task_reg_decay', type=float, default=1.0, help='Task regularizer coefficient for exponential decay per batch')
    ...
    parser.add_argument('--mask_type', type=int, default=0, help='masking strategy')

    parser.add_argument('--period', type=int, default=5, help='exchange period btw de-cent clients')
...
def load_data(args, dataset_name):
    if (args.dataset != 'freesolv') and (args.dataset != 'qm9') and (args.dataset != 'esol') and (args.dataset != 'lipo') and (args.dataset != 'herg') \
    ...
    train_data_local_num_dict, train_data_local_dict, val_data_local_dict, test_data_local_dict = load_partition_data(
        args,
        args.data_dir,
        args.client_num_in_total,
        uniform= unif, compact=compact, normalize_features=args.normalize_features, normalize_adj=args.normalize_adjacency)

    if args.dataset == 'qm9' or args.dataset == 'qm8':
        args.metric = 'mae'
    else:
        args.metric = "rmse"
    args.loss= torch.nn.MSELoss(reduction='none') if args.dataset != 'qm9' else torch.nn.L1Loss(reduction='none')
...
if __name__ == "__main__":
    ...
    str_process_name = "SpreadGNN:" + str(process_id)
    ...
    if process_id == 0:
        wandb.init(
            project="SpreadGNN",
            name="SpreadGNN" +  args.model + "_" + args.dataset,
            config=args
        )
    ...
    [train_data_num, val_data_num, test_data_num, train_data_global, val_data_global, test_data_global,
     train_data_local_num_dict, train_data_local_dict, val_data_local_dict, test_data_local_dict] = dataset

    # create model.
    # Note if the model is DNN (e.g., ResNet), the training will be very slow.
    # In this case, please use our FedML distributed version (./fedml_experiments/distributed_fedavg)
    model = create_model(args, args.model, feat_dim, num_cats, output_dim=None)

    # start "federated averaging (FedAvg)"
    FedML_SpreadGNN_distributed(process_id, worker_number, device, comm,
