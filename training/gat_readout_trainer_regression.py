...
class GatMoleculeNetTrainer(ModelTrainer):
    ...
    def train(self, train_data, device, args):
        ...
        criterion = torch.nn.MSELoss(reduction='none') if args.dataset != 'qm9' else torch.nn.L1Loss(reduction='none')
        ...
        for epoch in range(args.epochs):
            avg_loss = 0
            count = 0
            for mol_idxs, (adj_matrix, feature_matrix, label, _, cli_mask) in enumerate(train_data):
                optimizer.zero_grad()
                if cli_mask is not None:
                    cli_mask = cli_mask.to(device=device, dtype=torch.float32, non_blocking=True)
                    if torch.all(cli_mask == 0).item():
                        continue
                ...
                pred_loss = criterion(logits, label) * cli_mask if cli_mask is not None else criterion(logits, label)
                pred_loss = pred_loss.sum() / cli_mask.sum() if cli_mask is not None else pred_loss.mean()
                
                if args.is_mtl:
                    W = self.model.readout.output.weight
                    lhs = torch.mm(W.t() , torch.inverse(self.model.omega_corr) )
                    trace_in = torch.mm(lhs, W)
                    loss = pred_loss +  self.task_reg * torch.trace(trace_in) + args.wd * 0.5 * torch.norm(W)**2
                    loss.backward()
                    optimizer.step()
                    with torch.no_grad():
                        W = self.model.readout.output.weight
                ...
                if ((mol_idxs + 1) % args.frequency_of_the_test == 0) or (mol_idxs == len(train_data) - 1):
                    if test_data is not None:
                        ...
                        if args.metric != 'r2':
                            print('Epoch = {}, Iter = {}/{}: Test {} = {}'.format(epoch, mol_idxs + 1, len(train_data), args.metric.upper(),test_score))
                            ...
                            print('Epoch = {}, Iter = {}/{}: Test R2 = {}'.format(epoch, mol_idxs + 1, len(train_data), test_score))
                            if test_score > min_score:
                                min_score = test_score
                                best_model_params = {k: v.cpu() for k, v in model.state_dict().items()}
    ...
    def test(self, test_data, device, args):
        ...
        with torch.no_grad():
            y_pred = []
            y_true = []
            for mol_idx, (adj_matrix, feature_matrix, label, _, _) in enumerate(test_data):
                adj_matrix = adj_matrix.to(device=device, dtype=torch.float32, non_blocking=True)
                feature_matrix = feature_matrix.to(device=device, dtype=torch.float32, non_blocking=True)
                label = label.to(device=device, dtype=torch.float32, non_blocking=True)
                logits = model(adj_matrix, feature_matrix)
